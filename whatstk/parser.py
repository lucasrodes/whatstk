...
# %%
# TODO: document
def user_interventions(chat: "WhatsAppChat", timestep: str = 'days',
                       length: bool = False) -> pd.DataFrame:
    if timestep == 'days':
...
# %%
def response_matrix(chat: pd.DataFrame,
                    ptype: str = 'absolute') -> pd.DataFrame:
    """
...
class WhatsAppChat:
    def __init__(self, filename, regex='default', date_format='dmy'):
        ...
        regex_dict = {
            'default': (
            '(\[)?\d{,4}.\d{,4}.\d{,4},? \d?\d:\d\d(:\d\d)?( )?([AaPp][Mm])?( )?[-:\]] [^:]*: ',
            '(\[)?\d{,4}.\d{,4}.\d{,4},? \d?\d:\d\d(:\d\d)?( )?([AaPp][Mm])?( )?[-:\]]'),
            'regex_1': (
            '\d?\d.\d?\d.\d{,4}, \d?\d:\d\d(:\d\d)?( )?([AaPp][Mm])?( )?[-:] [^:]*: ',
            '\d?\d.\d?\d.\d{,4}, \d?\d:\d\d(:\d\d)?( )?([AaPp][Mm])?( )?[-:]'),
        }
        ...
        if type(regex) == str:
            if regex in regex_dict.keys():
                ...
                print(
                    "Please type your regex as a tuple ('regex','regex alert')")
