...
def get_days(data):
	...
	# From [1]
	days = [list(x) for x in set(tuple(x) for x in days_rep)]
	days = sorted(dates,key=itemgetter(2,1,0))
	return days
...
def get_intervention_table_days(names, dates, data):
...
def get_intervention_table_hours(names, dates, data):
	# Put dates into nice visual format
	format_dates = nice_format_dates(dates)
	interventions_per_hours = np.zeros(len(dates))
